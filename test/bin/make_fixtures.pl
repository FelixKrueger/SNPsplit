...
for my $name (sort keys %F) {
    ...
    if ($spec->{generate}) {
        generate_annotation($dir, $cfg, $plain, $masked);
    }
    else {
...
sub generate_annotation {
    ...
    ### Sorted by position: the archive is written from a hash, so its line order differs on every run
    ### and an unsorted copy would dirty git diff each time this script is run.
    ...
    my $annotation = join('', map { "$_\n" }
        sort { (split /\t/, $a)[2] <=> (split /\t/, $b)[2] } @rows);
    write_file(File::Spec->catfile($dir, 'snps.txt'), $annotation);

    printf "%-20s generated from %s\n", 'gp_generated', 'SNPsplit_genome_preparation';
}
