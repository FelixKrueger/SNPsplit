...
### The reference and the SNP annotation are produced by running SNPsplit_genome_preparation, not by
### snp_table and fasta, so the format contract between the two tools is asserted rather than assumed.
### Same reasoning as the MD tags: derive it, commit it, and drift becomes a test failure.
###
### No unlisted position, because genome preparation masks exactly what it filters and cannot produce
### one - which is why this fixture carries its own reference rather than sharing %STD.
my %GP = (
    len  => 320,
    snps => { 30 => ['A','G'], 60 => ['T','C'], 130 => ['G','T'], 200 => ['C','A'] },
    unlisted => {},
...
$F{gp_generated} = {
    ...
    ref      => \%GP,
    generate => 'archive',
    notes    => 'SNP annotation and N-masked reference generated by SNPsplit_genome_preparation, so the format the two tools share is asserted rather than assumed',
    reads    => [
        { q => 'r_g1',       pos => 6,   subs => { 30 => 'A' } },
        { q => 'r_g2',       pos => 6,   subs => { 30 => 'G' } },
        { q => 'r_neither',  pos => 6,   subs => { 30 => 'C' } },
        { q => 'r_two_g1',   pos => 20,  subs => { 30 => 'A', 60 => 'T' } },
        { q => 'r_conflict', pos => 20,  subs => { 30 => 'A', 60 => 'C' } },
        { q => 'r_snp3_g2',  pos => 110, subs => { 130 => 'T' } },
        { q => 'r_snp4_g1',  pos => 180, subs => { 200 => 'C' } },
        { q => 'r_no_snp',   pos => 230 },
    ],
};

### The other format genome preparation writes: SNPs_<strain>/chr1.txt, which opens with a >chr header
### line and carries a sixth column. SNPsplit could not read it at all until #107 - the header was
### parsed as a SNP record and stored under an empty chromosome name.
###
### Same reference and same reads as gp_generated, so the two fixtures' allele assignments must agree.
$F{gp_per_chrom} = {
    args     => '--single_end',
    ref      => \%GP,
    generate => 'per_chrom',
    notes    => 'the per-chromosome SNP file written by SNPsplit_genome_preparation, header line and sixth column included, read as-is',
...
### #116: every BAM writer is a pipe to samtools, so close is the only place a write failure can be
### seen - the pipe opens before samtools runs and its stderr is discarded. tag2sort used to close four
### of eleven handles, none of them checked, so a truncated BAM was reported as success.
###
### The staged samtools fails only when writing genome1, so the run reaches the writers normally and
### exactly one of them fails.
$F{bam_write_fails} = {
    args       => '--single_end --samtools_path <WRITE_FAIL_SAMTOOLS>',
    notes      => 'a samtools that fails while writing genome1 aborts the run instead of leaving a truncated BAM and reporting success',
    reads      => $F{se_basic}{reads},
};
...
for my $name (sort keys %F) {
    ...
    if ($spec->{generate}) {
        generate_annotation($dir, $cfg, $plain, $masked, $spec->{generate});
    }
    else {
...
### Runs SNPsplit_genome_preparation on a synthetic VCF and takes the fixture's reference and SNP
### annotation from what it wrote.
###
### Two things are asserted here rather than trusted. The masked sequence must match this script's own
### independent model of N-masking, which is what makes the fixture a cross-check and not just a copy.
### And the annotation is taken from all_SNPs_*.txt.gz rather than SNPs_<strain>/chr*.txt, because the
### per-chromosome file opens with a >chr header line that SNPsplit's read_snps has no skip for - it
### would be read as a SNP record and produce a bogus hash key.
sub generate_annotation {
    my ($dir, $cfg, $plain, $masked, $format) = @_;
    $format ||= 'archive';

    my $gp = File::Spec->catfile($test_root, '..', 'SNPsplit_genome_preparation');
    die "genome preparation script not found at $gp\n" unless -x $gp;

    my $work = tempdir('gp_gen_XXXXXX', DIR => ($ENV{TMPDIR} || '/tmp'), CLEANUP => 1);
    make_path(File::Spec->catdir($work, 'genome'));
    write_file(File::Spec->catfile($work, 'genome', 'chr1.fa'), fasta($plain));

    my $text = "##fileformat=VCFv4.2\n##contig=<ID=chr1,length=$cfg->{len}>\n"
             . join("\t", '#CHROM','POS','ID','REF','ALT','QUAL','FILTER','INFO','FORMAT','GP') . "\n";
    for my $pos (sort { $a <=> $b } keys %{ $cfg->{snps} }) {
        my ($ref, $alt) = @{ $cfg->{snps}{$pos} };
        die "VCF REF $ref disagrees with the reference at $pos\n"
            unless substr($plain, $pos - 1, 1) eq $ref;
        $text .= join("\t", 'chr1', $pos, '.', $ref, $alt, '.', 'PASS', 'AC=2', 'GT:PL:DP:FI',
                      '1/1:0,10,20:5:1') . "\n";
    }
    write_file(File::Spec->catfile($work, 'snps.vcf'), $text);

    my $cwd = getcwd();
    chdir $work or die "chdir $work: $!\n";
    my $log = `SNPSPLIT_NO_SLEEP=1 '$gp' --vcf_file snps.vcf --reference_genome genome --strain GP 2>&1`;
    my $rc  = $?;
    chdir $cwd or die "chdir back: $!\n";
    die "genome preparation failed:\n$log\n" if $rc != 0;

    ### The chromosome is called chr1, and write_SNP_chromosome prefixes the name with another 'chr'.
    my $produced = File::Spec->catfile($work, 'GP_N-masked', 'chrchr1.N-masked.fa');
    die "genome preparation wrote no N-masked chromosome\n" unless -e $produced;

    my $seq = slurp_seq($produced);
    die "N-masking disagrees with this script's model of it:\n"
      . "  generated: " . length($seq)  . " bp\n"
      . "  expected:  " . length($masked) . " bp\n"
        unless $seq eq $masked;

    copy_text($produced, File::Spec->catfile($dir, 'chr1.fa'));

    my $annotation;
    if ($format eq 'per_chrom') {
        ### Taken verbatim, header line and sixth column included: that is the point of the fixture.
        ### Written in VCF order, so no sorting is needed.
        ### Globbed rather than named: write_SNP_chromosome prefixes the chromosome name with 'chr', so
        ### a reference chromosome called chr1 lands in a file called chrchr1.txt
        my ($per_chrom) = glob File::Spec->catfile($work, 'SNPs_GP', '*.txt');
        die "genome preparation wrote no per-chromosome SNP file\n" unless $per_chrom;
        $annotation = slurp_text($per_chrom);
        die "the per-chromosome SNP file is empty\n" unless length $annotation;
        die "the per-chromosome file no longer opens with a header line\n"
            unless $annotation =~ /\A>/;
    }
    else {
        ### Sorted by position: the archive is written from a hash, so its line order differs on every
        ### run and an unsorted copy would dirty git diff each time this script is run.
        my $archive = File::Spec->catfile($work, 'all_SNPs_GP_GRCm39.txt.gz');
        my @rows    = grep { /\S/ } split /\n/, `gunzip -c '$archive'`;
        die "genome preparation wrote no SNP archive\n" unless @rows;
        $annotation = join('', map { "$_\n" }
            sort { (split /\t/, $a)[2] <=> (split /\t/, $b)[2] } @rows);
    }
    write_file(File::Spec->catfile($dir, 'snps.txt'), $annotation);

    printf "%-20s generated from %s (%s)\n", basename($dir), 'SNPsplit_genome_preparation', $format;
}
...
sub slurp_seq {
    ...
    open my $fh, '<', $file or die "read $file: $!\n";
    my $seq = '';
    while (<$fh>) { next if /^>/; chomp; $seq .= uc $_ }
    close $fh;
    return $seq;
}
